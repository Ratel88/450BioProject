...
public class PlotFrame extends JInternalFrame {
  ...
  private JMenu zoomingMenu = new JMenu();
  private JMenuItem reset = new JMenuItem();
  private JMenuItem zoomHelpChoice = new JMenuItem();
  ...
  private ActionListener geneListener; //listens for changes in selected genes
  private Project project;
    ...
    private void jbInit () throws Exception {
      ...
      zoomHelpChoice.setText("Zooming Help");
      zoomHelpChoice.addActionListener(new java.awt.event.ActionListener() {
    	public void actionPerformed(ActionEvent e) {
    		zoomHelpChoice_actionPerformed(e);
    	}
      });
      ...
      selectAll.setText("Select All Genes");
      ...
      zoomingMenu.add(undoZoom);
      zoomingMenu.add(reset);
      zoomingMenu.add(zoomHelpChoice);

      jMenu4.add(normChoice);
      jMenu4.add(origData);
      jMenu4.add(regress);
      jMenu4.add(selectAll);
    ...
    public void setUpPlot(){
      ...
      if(xlabels!=null&&!columnPlot)plotPanel.setXLabels(xlabels);
      else plotPanel.clearXLabels();
      if(columnPlot) plotPanel.setGraphSize(txmin,txmax,ymin,ymax);
      else plotPanel.setGraphSize(0,(expMain.getColumns()==1?2:expMain.getColumns()-1),ymin,ymax);
      //plotPanel.title=expMain.name; //Sets title to short title. WHY ARE WE DOING THIS?
      if (plotPanel.title == null || plotPanel.title.equals("")){
    	  System.out.println("PlotPanel.title is null or blank.");
    	  plotPanel.title=expMain.name; 
      }
    ...
    //saves a gif image of the graph
...
//repaint here
    ...
    //resets the graph to the original view
    ...
    private void zoomHelpChoice_actionPerformed(ActionEvent e) {
    	JOptionPane.showMessageDialog(parentFrame, "To zoom in on the plot, click and drag the mouse while holding the Control key.\nA blue box will be drawn on the plot showing the area into which you are zooming.\nWhen you release the mouse button or the Control key, the plot will zoom in on the specified area.","MAGIC Tool Information",JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void selectAll_actionPerformed(ActionEvent e){
    		plotPanel.selectAllGenes();
    		Vector v=new Vector();
    		for (int i=0; i<geneSelector.getItemCount(); i++){
    			v.addElement(geneSelector.getItemAt(i).toString());
    		}
    		genePanel.setGene(v);
    }

    //returns the data to the original values - undoes normalizing the data
    ...
    //changes the shape of the points
    ...
    //allows the user to select genes in a certain chromosome
    ...
    //allows the user to select genes whose comments contain a specified string
    ...
    //allows the user to select genes whose biological function contain a specified string
    ...
    //allows the user to select genes whose Molecular Process contain a specified string
    ...
    //used to determine if undoing zoom is possible
    ...
    //changes the x and y location values when the mouse moves on the graph
    private void pp_mouseMoved(MouseEvent e) {
      ...
      if(plotPanel.inGraph(x,y)){
        ...
        if(!plotPanel.useXLabels)position.setText("x:"+xformat.format(plotPanel.xPosToValue(x))+ " y:" + yformat.format(plotPanel.yPosToValue(y)));
        else{
          int closest;
          if(x<plotPanel.xValueToPos(0)) closest=0;
          else if((x>plotPanel.xValueToPos(plotPanel.xMarks.length-1))) closest=plotPanel.xMarks.length-1;
          else closest = Math.round((float)plotPanel.xPosToValue(x));
    ...
    //changes the selected gene on the graph when the user picks a gene from the combobox
