 ...
 public class AverageDataDialog extends JDialog{
  ...
  private void okButton_actionPerformed(ActionEvent e) {
	  ...
	  if(okButton.isEnabled()){
		  //final ProgressFrame progress = new ProgressFrame("Loading Info File", false, this);
					  ...
					  if(allGenes.contains(exp1.getGene(exp1.findGeneName(name)))){
						  if(name.toLowerCase().indexOf("_rep")!=-1){
							  ...
							  for(int j=1; j<allReps.size(); j++){
								  ...
								  for(int k=0; k<avgData.length; k++)
									  avgData[k]+=data[k];
							  }


							  for(int k=0; k<avgData.length; k++)
								  avgData[k]=avgData[k]/(double)allReps.size();

							  for(int j=0; j<allReps.size(); j++)
								  allGenes.remove(allReps.get(j));
