...
public class GriddingFrame extends JInternalFrame {
  ...
  protected JToggleButton zoomIn = new JToggleButton();
  protected JToggleButton zoomOut = new JToggleButton();
  private JButton reset = new JButton();
  private JButton done = new JButton();
  private RotateButtonPanel rotateButtonPanel = new RotateButtonPanel();
  protected DecimalFormat df = new DecimalFormat("###.#");
  protected JLabel statusBar = new JLabel();
  private TitledBorder titledBorder1;
  private JSplitPane jSplitPane1 = new JSplitPane();
  private JPanel jPanel1 = new JPanel();
  protected JTabbedPane gridTabs = new JTabbedPane();
  ...
  private GridPanel[] gridPanel; //panels in the grid tabs
  ...
  /**This action listener takes care of moving the grid around*/
  protected static GridMoverAdapter gma;
   ...
   */
  public GriddingFrame(Project project, GridManager m, String redPath, String greenPath, MainFrame main) {
    this.manager = m;
    this.main=main;
    ...
    if(manager.getNumGrids()==0){
      manager.setGridNum(1);
      manager.setCurrentGrid(0); //For drawing we need to set this.
      doFinal=true;
    }
    else doFinal=false;
    titles = new String[manager.getNumGrids()];
    gridPanel = new GridPanel[manager.getNumGrids()];
  ...
  private void jbInit() throws Exception {
    ...
    rotateButtonPanel.setMaximumSize(new Dimension(250, 30));
    rotateButtonPanel.setMinimumSize(new Dimension(250, 30));
    rotateButtonPanel.setPreferredSize(new Dimension(250, 30)); 
 
    for (int i=0; i<titles.length; i++) {
        titles[i] = String.valueOf(i+1);
        gridPanel[i] = new GridPanel(manager.getGrid(i), this.getDisplay());
        gridTabs.add(gridPanel[i], titles[i]);
    }

    gridTabs.addChangeListener(new ChangeListener() {
      public void stateChanged(ChangeEvent ce) {
        JTabbedPane source = (JTabbedPane)ce.getSource();
        manager.setCurrentGrid(source.getSelectedIndex());
    ...
    //imageToolBar.add(rotateButtonPanel);
 ...
 //   jPanel2.add(contrastInputFieldLabel, null);
    ...
    //optionsMenu.add(undoMenu); //Removed, undo does not make much sense in the context of the new gridding scheme.
    ...
    gridTabs.addChangeListener(new ChangeListener(){
        ...
        public void stateChanged(ChangeEvent e){
          ((GridPanel)gridTabs.getSelectedComponent()).resetButtons();
          ((GridPanel)gridTabs.getSelectedComponent()).updateApplyFromBox();
        }
    });
    ...
    gma = new GridMoverAdapter(this,imageDisplayPanel);
    imageDisplayPanel.ic.addMouseListener(gma);
    imageDisplayPanel.ic.addMouseMotionListener(gma);
    imageDisplayPanel.ic.setFocusable(true);
...
//    this.addKeyListener(new GridCopyListener(manager));
//    this.getContentPane().addKeyListener(new GridCopyListener(manager));
    this.getContentPane().setFocusable(true); this.getContentPane().requestFocus();
  ...
  public GridPanel currentGridPanel() {
    if (gridTabs.getSelectedIndex()>=0 && gridTabs.getSelectedIndex()<gridPanel.length) return gridPanel[gridTabs.getSelectedIndex()];
    else return null;
  }
  ...
  /**
   * This method uses the getTranslated Polygon to see if all of the grids are inside of the image.
   * This prevents future problems at segmentation.
   */
  public boolean gridOutside(){
	  Rectangle r;
	  for(int i=0; i<manager.getNumGrids();i++){
		  r = manager.grids[i].getTranslatedPolygon().getBounds();
		  if(r.x<0 || r.y<0 || r.x+r.width>width || r.y+r.height>height){	  
			  JOptionPane.showMessageDialog(this, "Grid " + ++i + " is off the image.", "Grid Off the Image Error",
  ...
  /**
   * This method tests that two grids do not intersect. If they did then a problem would occur during 
   * spot flagging.
   * @return
   */
  public boolean gridOverlap(){
	  for(int i=0; i<manager.getNumGrids()-1;i++){
		  for(int j=i+1; j<manager.getNumGrids();j++){
			  if(Grid.gridIntersections(manager.grids[i],manager.grids[j])){
				  JOptionPane.showMessageDialog(this, "Grid " + ++i + " intersects with grid " + ++j + ".", "Grid Overlap Error",
						  JOptionPane.ERROR_MESSAGE);				  
				  return true;
			  }
		  }
	  }
	  //No overlap...we can continue as normal.
	  return false;
  }
  ...
  /**
   * brings a menu to allow users to change the grid properties
   */
  public void changeProperties(){
      GridOptionsDialog god = new GridOptionsDialog(main);
  ...
  /* Andrew Martens. finalInit will only be called if a new gridding frame is being created. */
  ...
  public void finalInit(){
	  ...
	  if(doFinal) {
        ...
        String multiLineWarningMsg[] = {"You must understand your genelist configuration before you start gridding.","The first grid you create must contain spot number 1, corresponding to the first gene in the genelist."};
        JOptionPane.showMessageDialog(this.getDesktopPane(), multiLineWarningMsg, "Warning!", JOptionPane.INFORMATION_MESSAGE);
        statusBar.setText("Specify Grid Options...");
        final GridOptionsDialog god = new GridOptionsDialog(main);
  ...
  //This is used when we have created metagrids, and want to extract regular grid info from these
  public void fromMetaInit( int numGr, boolean LR, boolean TB, boolean GD) {
      ...
      //with these values, it will suggest the correct # of grids and so on
      god.setOptions(numGr, LR, TB, GD);
      god.setModal(true);
      god.pack();
      god.setVisible(true);
	  ...
	  //gridoptionsdialog has the right info, once user OKs then we have to automagically pull in appropriate
      //metagridding info after they click one grid
	  //this will be checked for in update button of GridPanel
  ...
  private void done_actionPerformed(ActionEvent e) {
    ...
    if(canSegment() && !gridOutside() && !gridOverlap()){
      JOptionPane.showMessageDialog(this, "You Have Gridded And Named " + manager.getTotalSpots() + " Genes.");
      setUpSpotObjects();
      if(needSaved){
        cont = JOptionPane.showConfirmDialog(this, "You Have Altered Your Grid Data. Current Grid Data Will Only Be Available\n During This MAGIC Tool Session. Would You Like To Permanently Save Data?", "Save Data?", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
      }
      if(cont==JOptionPane.YES_OPTION){
        if(saveGridMenu.isEnabled()) saveGridMenu_actionPerformed(e);
        else  saveAsGridMenu_actionPerformed(e);
      }
      if(cont!=JOptionPane.CANCEL_OPTION) this.dispose();
    }
    else{
      if(manager.getNumGrids()<1){
        cont = JOptionPane.showConfirmDialog(this, "Error! You Have Not Defined Any Grids\nYou Cannot Not Begin Segmentation Until These Numbers Match\nDo You Still Wish To Close Gridding?", "Error!", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
        if(cont==JOptionPane.YES_OPTION){
         this.dispose();
       }
       if(cont!=JOptionPane.NO_OPTION){}
      }
      else if(!manager.gridsSet()) {
        cont = JOptionPane.showConfirmDialog(this, "Error! You Have Not Defined All Grids\nYou Cannot Not Begin Segmentation Until These Numbers Match\nDo You Still Wish To Close Gridding?", "Error!", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
        if(cont==JOptionPane.YES_OPTION){
           this.dispose();
      }
       if(cont!=JOptionPane.NO_OPTION){}
      }
      else if(manager.getTotalSpots()!=manager.getGeneListSize()){
       cont = JOptionPane.showConfirmDialog(this, "Error! You Have Gridded " + manager.getTotalSpots() + " Genes.\nYour Gene List Contain " + manager.getGeneListSize() + "Genes.\nYou Cannot Not Begin Segmentation Until These Numbers Match\nDo You Still Wish To Close Gridding?", "Error!", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
       if(cont==JOptionPane.YES_OPTION){
           this.dispose();
      }
       if(cont!=JOptionPane.NO_OPTION){}
  ...
  private void contrastInputField_actionPerformed(ActionEvent e) {
	  int inputContrast = (int)Double.parseDouble(contrastInputField.getText());		
	  if(inputContrast > 1000000){
		  //Value too large and silly, computer will run out of memory.
		  JOptionPane.showMessageDialog(this, "Contrast value too large. \nPlease use a smaller value.", "Error", 
				  JOptionPane.ERROR_MESSAGE);
	  }else if(inputContrast<=3){
		  JOptionPane.showMessageDialog(this, "Contrast value too small. \nPlease use a larger value.", "Error", 
				  JOptionPane.ERROR_MESSAGE);	
	  }else{	  
		  //Standard Case.
...
/*
  ...
  */
      ...
      if (result == JFileChooser.APPROVE_OPTION) {
          ...
          try{
              ...
              //if(!open){
                ...
                needSaved=false;
                addButtonListeners();
                setUpSpotObjects();	//added 6-4-2007 MRG
              //}
                if (!canSegment()) {
                	JOptionPane.showMessageDialog(this, "Error: The loaded grid does not have the same number of grid squares as there are genes in your gene list.\nThe grid has been loaded, but you will not be able to continue to segmentation until these errors are corrected.","MAGIC Tool Error",JOptionPane.ERROR_MESSAGE);
                }
  ...
  private void undoMenu_actionPerformed(ActionEvent e) {
    if(lastUpdateGrid>=0&&lastUpdateGrid<manager.getNumGrids()){
      manager.setGrid(lastUpdateGrid, new Grid(GridPanel.lastUpdate[0], GridPanel.lastUpdate[1],GridPanel.lastUpdate[2], GridPanel.lastUpdate[3],GridPanel.lastUpdate[4], GridPanel.lastUpdate[5],GridPanel.lastUpdate[6], GridPanel.lastUpdate[7],GridPanel.lastUpdate[8], GridPanel.lastUpdate[9]));
      gridPanel[lastUpdateGrid].setDataFromGrid(manager.getGrid(lastUpdateGrid));
      manager.setCurrentGrid(lastUpdateGrid);
      imageDisplayPanel.repaint();
    }
    undoMenu.setEnabled(false);
    lastUpdateGrid=-1;
  ...
  /**
   * Sets up all the Spot objects, which will be used by the FlagEditFrame.
   *
   */
  public void setUpSpotObjects() {
	  int numGrids = manager.getNumGrids();
	  int rowLength = -1;
	  int colLength = -1;
	  int index = 0;
	  Spot[] spots = manager.getSpots();
	  if (numGrids > 0)
	  {
		  for(int g=0; g<numGrids;g++) {
			  rowLength = manager.getGrid(g).getRows();
			  colLength = manager.getGrid(g).getColumns();
			  for(int i = 0; i<rowLength; i++) {
				  for(int j= 0; j<colLength;j++) {
					  String name = manager.getGeneName(g, j,i);
					  try{
						  spots[index] = new Spot(name, g, manager.getTransformedSpotNum(g,j,i));
						  index++;
					  }
					  catch(Exception e) {
						  e.printStackTrace();
						  System.out.println("This appears to be an incomplete grid -- in magictool.image.GriddingFrame.setUpSpotObjects()");
						  if (g < 0) System.out.println("that");
						  return;
						  //if (spots[index] == null) System.out.println("other that");
					  }
				  }
			  }
		  }
  ...
  protected void setLastUpdatedGridNumber(int n){
	  lastUpdateGrid=n;
  }
  
  /**
   * This class displays and organizes the rotation buttons properly.
   */
  protected class RotateButtonPanel extends JPanel {
	private static final long serialVersionUID = 1L;
	JLabel title = new JLabel("Rotate Grid", JLabel.CENTER);
	JPanel buttonPanel = new JPanel(new FlowLayout());
	JButton counterclockwiseButton = new JButton(new ImageIcon(MagicToolApp.class.getResource("gifs/Counterclockwise.gif")));
	JButton clockwiseButton = new JButton(new ImageIcon(MagicToolApp.class.getResource("gifs/Clockwise.gif")));
	  
	  /**
	   * This sets up the buttons and panels in the appropriate places.
	   */
	  public RotateButtonPanel(){
