 ...
 */
class MetaGridPanel extends JPanel {
   ...
   */
  public MetaGridPanel(MetaGrid g, ImageDisplayPanel id) {
      this.iDisplay = id;
      this.grid = g;
  ...
  private void jbInit() throws Exception {
    ...
    rowNumLabel.setText("Meta-Rows");
    rowNum.setToolTipText("");
    rowNum.setColumns(6);
    this.setPreferredSize(new Dimension(200, 460));
    columnNumLabel.setText("Meta-Columns");
    ...
    tlButton.setToolTipText("Click on the top-left corner of the metagrid.");
    tlButton.setText("Set Top Left Corner");
    trButton.setToolTipText("Click on the top-right corner of the metagrid.");
    trButton.setActionCommand("Set Top Right");
    trButton.setText("Set Top Right Corner");
    ...
    bottomRowPt.setToolTipText("Click on the bottom of the bottom row of the metagrid.");
  ...
  public void updateGrids(){
      try{
          ...
          if(rows>0&&columns>0){
        	  ...
        	  //Calculate the corners for the rectangle that we will draw around the metagrid
   ...
   */
  public void setDataFromGrid(MetaGrid g){
    this.grid=g;
    tlSpotX = g.topLeftX;
    tlSpotY = g.topLeftY;
    trSpotX = g.topRightX;
  ...
  protected void updateApplyFromBox(){
    ...
    int n=0;
    int pos=0;
    for(int i=0; i<iDisplay.metaManager.numMetaGrids; i++){
      if(iDisplay.metaManager.getMetaGrid(i).isValid()&&iDisplay.metaManager.getMetaGrid(i)!=grid){
    ...
    //!!! note: error sometimes here: out of bounds?
    applyFrom.setSelectedIndex(pos);
  }


  //gets the grid to apply from
  private MetaGrid applyFromGrid(int gridNum){
    return (iDisplay.metaManager.getMetaGrid(gridNum-1));
  }
