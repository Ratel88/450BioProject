...
public class MetricTreeTableFrame extends JInternalFrame {
  ...
  public MetricTreeTableFrame(File clustFile, ExpFile exp, Frame parentFrame, Project project) {
    ...
    try{
      ...
      if(info[5].equals("Hierarchical"))useTree=true;
      else useTree=false;
    }

    catch(Exception e){}
    if(useTree) side = new MetricTree(clustFile,false,false,4,colwidth);
    else side = new DissimListPanel(clustFile,4);
  ...
  private void jbInit() throws Exception {
    ...
    if(useTree) ((MetricTree)side).setLineHeight(10);
    else ((DissimListPanel)side).setLineHeight(10);
  ...
  //changes the height of a each cell
  private void heightButton_actionPerformed(ActionEvent e) {
    try{
      ...
      if(useTree) ((MetricTree)side).setLineHeight(i);
      else ((DissimListPanel)side).setLineHeight(i);
      table.paintTable(minvalue,centervalue,maxvalue, table.getRowHeight());
