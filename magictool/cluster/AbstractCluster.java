...
public abstract class AbstractCluster extends Thread implements Cancelable, Executable {
   ...
   //Variables determined by .dis file
   /**number of genes in dissimilarity file*/
   protected int numGenes=0;       // number of objects to cluster
   /**total number of dissimilarities listed in dissimilarity file*/
   protected int numPairs=0;       // number of entries in sim matrix
   /**dissimilarity matrix from dissimilarity file*/
   protected float dys[][]=null;      // array for holding similarity data
   /**labels for the genes*/
   //protected ArrayList labels=null;                   // contains the labels with spaces
   /**new gene labels field.  To replace labels.*/
   protected String[] geneLabels = null;
   /**file path for the original expression file*/
   protected String expFilePath=""; // original expression file
   ...
   public void cancel(){
      ...
      if(f.exists()){
         ...
         //removes file from the project if it previously existed
   ...
   /**
    * @param filePath file path of serialized .dis file (SerialDiss.java)
    * @return instance of SerialDiss()
    */
   public SerialDiss deSerialize(String filePath) throws IOException, ClassNotFoundException{
      ObjectInputStream in = new ObjectInputStream(new FileInputStream(filePath));
      SerialDiss s = (SerialDiss)in.readObject();
      return s;
   }
   
   public void readSerializedDissimilarityFile (String filePath, ProgressFrame progress) throws IOException{
      try {
         readSerializedDissimilarityFile(deSerialize(filePath), progress);
      } catch (ClassNotFoundException ex) {
         ex.printStackTrace();
      }
   }
   
   public void readSerializedDissimilarityFile(SerialDiss serial, ProgressFrame progress) throws IOException{
      try{
         numGenes = serial.getNumGenes();
         expFilePath = serial.getExpFilePath();
         dissimMethod = serial.getDisMethod();
         if (dissimMethod == 1) dissimParams = serial.getModifiers();
         geneLabels = serial.getGeneLabels();
         dys = serial.getFloats();
...
//		  /* old (safe) method of reading the .dis file
//		   *
//		   * DataInputStream stream = new DataInputStream(new BufferedInputStream(new FileInputStream(filename)));
//		   * readDisHeaders(stream); //reads the header of the dis file
//		   * if(progress!=null)progress.setTitle("Reading Dissimilarity File...");
//		   */
//
//		  FileChannel dysChannel = new FileInputStream(filename).getChannel();
//		  //might want to use a BufferedInputStream with the DataInputStream?  It messes up the file mark, though.
//		  DataInputStream dysHeaders = new DataInputStream( Channels.newInputStream(dysChannel) );
//		  readDisHeaders(dysHeaders);
//
//		  /*
//		   * Read in the labels (the gene names, really) from the beginning of the dis file.
//		   * An ArrayList is allocated with size equal to number of genes names,
//		   * and populated by calling stream.readUTF() once for each element of the
//		   * ArrayList.
//		   * Old Code:
//		   *    String labelstring = stream.readUTF();
//		   *    StringTokenizer tokenizer = new StringTokenizer(labelstring);
//		   */
//		  labels = new ArrayList(numGenes);
//		  for (int count = 0; count < numGenes; count++)
//			  labels.add(dysHeaders.readUTF());	//changed stream.readUTF() to dysHeaders.readUTF()
//
//		  //the mapped byte buffer: maps the dys file into the precices amount of non-javaVM memory necessary, starting where dysHeaders stopped.
...
//		  dysHeaders.close();	//NOTE: also implicitly closes dysChannel.  Supposedly.
//
//		  dys = new float[numGenes - 1][];
//		  //Calculate the number of similarity values to read
//		  numPairs = (numGenes*(numGenes - 1))/2;
...
//				  dys[row - 1][column] = dysBuffer.getFloat();	//changed stream.readFloat() to dysBuffer.getFloat()
...
//		  disReadTimer.finish();
//	  } catch (Exception e) {
//		  throw new IOException(); //if file cannot be read properly
//	  }
//  }
   
//reads the dissimilarity headers
    ...
    */
   public void start(){
      //resets variables for canceling and determing success of process
      cancel=false;
      completed=false;
      over=false;
    ...
    * [4] dissimilarity parameters <br>
    * [5] dissimilarity file <br>
    * [6] cluster method <br>
   ...
   public static String[] readHeaders(String clustFile) throws Exception{
      //System.out.println("clustFile: " + clustFile);
