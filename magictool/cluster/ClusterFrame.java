...
public class ClusterFrame extends JInternalFrame implements GeneCreator, KeyListener{
   ...
   public ClusterFrame(Project project, Frame parentFrame) {
      ...
      expComboBox = new FileComboBox(project,Project.EXP, "all", true, "Expression");
      disComboBox = new FileComboBox(project,Project.DIS, "all", true, "Dissimilarity");
      try {
         jbInit();
   ...
   private void jbInit() throws Exception {
      ...
      //initializes the frame and adds the listeners
   ...
   private void clusokButton_actionPerformed(ActionEvent e) {
      try{
         String out;
         boolean canDispose=true; //whether or not the frame can be disposed
         
         //gets cluster file path
         if(useDisFile) out = disComboBox.getFilePath();
         else out = expComboBox.getFilePath();
         ...
         //checks if the outfile is valid and then creates the cluster
         if (outfileIsValid(out)){
            if(clusterCombo.getSelectedItem().toString().equals("Hierarchical Clustering")) {
               ...
               if(canDispose) cluster.setProject(project); //sets the project
            }
            ...
            // dispose() here seems to cause a bunch of ArrayIndexOutOfBoundsExceptions.
            // MC:7/11/2005 fixed, see SwingUtilities.invokeLater(...)
            // Old Code:
            // if(canDispose) this.dispose(); //disposes the window if there were no errors
            if(canDispose) {
         ...
         //catches fields where numbers were not entered correctly
   ...
   private boolean disfileIsValid(String disfile) {
      ...
      if (disFile.isDirectory()) {
         JOptionPane.showMessageDialog(this, "The dissimilarity file path is a directory.  Please add a file name.",
                 "Directory Found", JOptionPane.OK_OPTION);
         return  false;
      } else if (!disFile.exists()) {
         JOptionPane.showMessageDialog(this, "The file " + disFile.getPath()
         + " does not exist.  Please select an existing file.",
   ...
   private boolean expfileIsValid(String expfile) {
      ...
      if (expFile.isDirectory()) {
         JOptionPane.showMessageDialog(this, "The expression file path is a directory.  Please add a file name.",
                 "Directory Found", JOptionPane.OK_OPTION);
         return  false;
      } else if (!expFile.exists()) {
         JOptionPane.showMessageDialog(this, "The file " + expFile.getPath()
         + " does not exist.  Please select an existing file.",
                 "File Does Not Exist", JOptionPane.OK_OPTION);
         return  false;
      }
   ...
   //returns whethr or not the outfile is valid
   private boolean outfileIsValid(String outfile) {
      ...
      if (outFile.isDirectory()) {
         ...
         int result = JOptionPane.showConfirmDialog(this, "The file "
   ...
   //sets the information about the cluster or expression file chosen to make the cluster file
   private void setInfo(int type) {
      if(type==1){
         ...
         try{
            //WTH 2/10/2007 RandomAccessFile stream = new RandomAccessFile(disComboBox.getFilePath(), "r");
        	   FileInputStream fis = new FileInputStream(disComboBox.getFilePath());
        	   ObjectInputStream stream = new ObjectInputStream(fis);
        	   SerialDiss serial = new SerialDiss();
        	   serial = (SerialDiss) stream.readObject();
             //WTH 2/10/2007 numIs.setText(Integer.toString(stream.readInt()));
            numIs.setText(Integer.toString(serial.getNumGenes()));
            String exp = "";
            //WTH 2/10/2007 if(stream.readBoolean()==true)
               //WTH 2/10/2007 exp =stream.readUTF();
            //WTH 2/10/2007 exp = (String)stream.readObject();
            exp = serial.getExpFilePath();
            //WTH 2/10/2007 expInfo.setText(exp.substring(exp.lastIndexOf(File.separator)+1));
            expInfo.setText(exp);        
            //WTH 2/10/2007 int method = stream.readInt();
            int method = serial.getDisMethod();
            if(method==0)
               methodInfo.setText("1 - Correlation");
            else if(method==1)
               methodInfo.setText("l^p p="+stream.readUTF());
            else if(method==2)
               methodInfo.setText("Jackknife correlation");
            else if(method==magictool.dissim.Dissimilarity.ABSCORR)
            	methodInfo.setText("1 - abs(Correlation)");
         } catch(Exception e2){
  ...
  //          numIs.setText("---");
   ...
   //changes the options display based on the clustering method chosen
   private void clusterCombo_itemStateChanged(ItemEvent e) {
      if(clusterCombo.getSelectedItem().toString().equals("Hierarchical Clustering")){
         cardParam.show(paramPanel, "hierPanel");
         cardFile.show(inputPanel, "disinfoPanel");
         useDisFile=true;
         if(disComboBox.getSelectedIndex()!=0) clusokButton.setEnabled(true);
         else clusokButton.setEnabled(false);
         ...
         useDisFile=false;
         if(expComboBox.getSelectedIndex()!=0) clusokButton.setEnabled(true);
         else clusokButton.setEnabled(false);
         ...
         if(disComboBox.getSelectedIndex()!=0) {
            ...
            else selectGeneButton.setEnabled(false);
      ...
      setOutFileField(); //sets the default clustering name
      
   }
   
   //changes the information shown based on the dissimlarity file chosen
   ...
   //changes the information shown based on the expression file chosen
   ...
   protected void setOutFileField(){
      if(useDisFile){
         if(disComboBox.getSelectedIndex()==0) outnameField.setText("");
         else{
         ...
         if(expComboBox.getSelectedIndex()==0) outnameField.setText("");
         else{
   ...
   //displays a gene creator frame to create a gene for Supervised QT Clustering
   ...
   //allows user to select a gene for Supervised QT Clustering
   ...
   private String getExpFile(){
      System.out.println(expInfo.getText());
