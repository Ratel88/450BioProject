...
public class TaskBuilderFrame extends JInternalFrame implements GeneCreator, KeyListener {
  ...
  protected static final Color BACKGROUND_COLOR = new Color(204, 204, 204);

  //file choosers for user to select from
  ...
  public TaskBuilderFrame(Project project, TaskManager manager, Frame parentFrame) {
    ...
    disComboBox = new FileComboBox(manager.getTaskProject(),Project.DIS,"all", false,"Dissimilarity");
    expBox = new FileComboBox(manager.getTaskProject(),Project.EXP,"all",false,"Expression");
  ...
  private void jbInit() throws Exception {
    ...
    jLabel1.setBackground(BACKGROUND_COLOR);
    ...
    jLabel3.setBackground(BACKGROUND_COLOR);
    ...
    qtPanel.setBackground(BACKGROUND_COLOR);
    ...
    numPane.setBackground(BACKGROUND_COLOR);
    ...
    jPanel12.setBackground(BACKGROUND_COLOR);
    ...
    jPanel6.setBackground(BACKGROUND_COLOR);
    this.setClosable(true);
    this.getContentPane().setBackground(BACKGROUND_COLOR);
    jPanel10.setBackground(BACKGROUND_COLOR);
    jPanel10.setLayout(verticalLayout6);
    clustPanel.setBackground(BACKGROUND_COLOR);
    clustPanel.setMaximumSize(new Dimension(494, 241));
    clustPanel.setMinimumSize(new Dimension(494, 241));
    jPanel8.setBackground(BACKGROUND_COLOR);
    jPanel8.setLayout(borderLayout6);
    jPanel9.setBackground(BACKGROUND_COLOR);

    executeBox.setBackground(BACKGROUND_COLOR);
    ...
    disPanel.setBackground(BACKGROUND_COLOR);

    expBox.setBackground(BACKGROUND_COLOR);
    ...
    jackPanel.setBackground(BACKGROUND_COLOR);
    ...
    absCorrPanel.setBackground(BACKGROUND_COLOR);
    absCorrButton.setBackground(BACKGROUND_COLOR);
    pLabel.setBackground(BACKGROUND_COLOR);
    pLabel.setText("p=");

    lpButton.setBackground(BACKGROUND_COLOR);
    ...
    corrButton.setBackground(BACKGROUND_COLOR);
    ...
    jLabel7.setBackground(BACKGROUND_COLOR);
    jLabel7.setText("Threshold");
    jPanel11.setBackground(BACKGROUND_COLOR);
    ...
    clusterCombo.setBackground(BACKGROUND_COLOR);

    fileChooserPanel.setBackground(BACKGROUND_COLOR);
    fileChooserPanel.setLayout(fileCard);
    disLabel.setBorder(border1);
    disLabel.setText("Dissimilarity File");
    disChooserPanel.setBackground(BACKGROUND_COLOR);
    ...
    jLabel5.setBackground(BACKGROUND_COLOR);
    jLabel5.setText("K (# of Clusters):");
    knumPanel.setBackground(BACKGROUND_COLOR);
    ...
    kmaxPanel.setBackground(BACKGROUND_COLOR);
    kmaxPanel.setLayout(flowLayout10);
    maxCycleLabel.setBackground(BACKGROUND_COLOR);
    maxCycleLabel.setText("Max Cycles:");
    jPanel3.setBackground(BACKGROUND_COLOR);
    gridLayout1.setColumns(2);
    jPanel5.setBackground(BACKGROUND_COLOR);
    calculateBox.setBackground(BACKGROUND_COLOR);
    ...
    //
    absCorrPanel.add(absCorrButton,null);
    jPanel15.add(absCorrPanel,null);
    //
    ...
    disbuttonGroup.add(absCorrButton);
  ...
  //selects a user specified gene for supervised qt clustering
  private void selectGeneButton_actionPerformed(ActionEvent e) {
    ...
    String select = (String)JOptionPane.showInputDialog(this, "Please Select A Gene",
      "Select One", JOptionPane.QUESTION_MESSAGE, null, exp.getGeneVector(), (selectedGene==null?exp.getGeneName(0):selectedGene));
    if(select!=null) selectedGene = select;
  }

  //opens the genecreatorframe to get values for a created gene for supervised qt clustering
  ...
  private void executeBox_itemStateChanged(ItemEvent e) {
    if(executeBox.getSelectedIndex()==0){
      ...
      if(expBox.getSelectedIndex()==0)addTaskButton.setEnabled(false);
      else addTaskButton.setEnabled(true);
    }
  ...
  //changes visible parameter selection based on clustering method
  private void clusterCombo_itemStateChanged(ItemEvent e) {
    if(clusterCombo.getSelectedItem().toString().equals("Hierarchical Clustering")){
        ...
        if(disComboBox.getSelectedIndex()!=0) addTaskButton.setEnabled(true);
        else addTaskButton.setEnabled(false);
      }
        ...
        if(disComboBox.getSelectedIndex()!=0) {
          ...
          if(geneCheckBox.isSelected())selectGeneButton.setEnabled(true);
          else {
          selectGeneButton.setEnabled(false);
          createButton.setEnabled(true);
          }
  ...
  private void expBox_itemStateChanged(ItemEvent e) {
    if(executeBox.getSelectedIndex()==0){
      if(expBox.getSelectedIndex()!=0) addTaskButton.setEnabled(true);
      else addTaskButton.setEnabled(false);
    }
    setOutFileField();
  }
  ...
  public void setOutFileField(){
    if(executeBox.getSelectedIndex()==0){
      if(expBox.getSelectedIndex()==0) disField.setText("");
      else{
        String name = expBox.getFileName();
        if(name!=null){
          String type="c";
          if(jackButton.isSelected()) type="j";
          else if(lpButton.isSelected()) type="lp" + pField.getText();
          else if(absCorrButton.isSelected()) type = "abs-c";
      ...
      if(disComboBox.getSelectedIndex()==0) clusterField.setText("");
      else{
  ...
  //checks to ensure that all parameters are properly specified and calls the taskfactory
  //to create the task and then sends the new task to the task manager
  ...
  //returns whether or not the outfile name is valid
  ...
  private void absCorrButton_itemStateChanged(ItemEvent e) {
	  if(absCorrButton.isSelected()) setOutFileField();
  }
