 ...
 *
 * $Author: nidovidio $
 * $RCSfile: KaryoColorPresetEditor.java,v $
 * $Revision: 1.1 $
 * $Date: 2007/06/01 14:33:18 $
 * $Name:  $
 *
...
/**
 * This class allows graphical editing of KaryoColorPresets
 * Also included is a class to pop up a configuration window.
 */
public class KaryoColorPresetEditor extends JPanel implements SettingsPanel {
     ...
     */
    public KaryoColorPresetEditor(KaryoColorPresets up) {
	  super();
	  presets = up;
...
//		 d.add(presetEditPanel);
	...
	final KaryoColorPresets p = new KaryoColorPresets(new DummyConfigNode("ColorPresets"));
	KaryoColorPresetEditor e  = new KaryoColorPresetEditor(p);
...
*/
	...
	private String title = "Karyoscope Color Preset Editor";
	  ...
	  private void initialize() {
		...
		*/
		private void initializePreset(int i) {
		  final int index = i;
		  final KaryoColorSet colorSet = new KaryoColorSet();
	  ...
	  public void redoLayout() {
		...
		*/
		...
		private void removePreset(int mark) {
		  KaryoColorSet [] tPresetColors = new KaryoColorSet[presetColors.length - 1];
		  ...
		  private void createPreset() {
			  KaryoColorSet toAdd = new KaryoColorSet();
			  addPreset(toAdd);
		  }
		  private void addPreset(KaryoColorSet toAdd) {
		  KaryoColorSet [] tPresetColors = new KaryoColorSet[presetColors.length + 1];
		  JTextField [] tPresetNames = new JTextField[presetNames.length + 1];
		  JRadioButton [] tDefaultButtons = new JRadioButton[defaultButtons.length + 1];
