 ...
 * @version    $Revision: 1.4.2.2 $ $Date: 2007/06/08 13:49:07 $
 */
public class LinkedViewApp extends TreeViewApp {
	...
	static LinkedViewFrame tvFrame = null; 
	static LinkedViewApp statView = null;
	...
	*/
	...
	//astring = new String[2];
	//astring[1]="/Users/nidovidio/Desktop/Sample Projects/BugFixProject/Hypothetical_4grid/Hypothetical_4gridh.cdt";

	/* inherit description */
	public ViewFrame openMagic(String [] astring) {
		// setup toplevel
		tvFrame  =
				new LinkedViewFrame(this);
		tvFrame.addWindowListener(this);
	...
	*/
	...
	protected void magicStartup(String astring[]){
			openMagic(astring).setVisible(true);
	}		
		
	
	
	protected void standardStartup(String astring[]) {
	//	System.out.println("Length: " + astring.length+ " value: " + astring[0]);
		int arg;
		//astring = new String[2];
		//astring[0]="-r";
		//astring[1]="/Users/nidovidio/Desktop/Sample Projects/BugFixProject/Hypothetical_4grid/Hypothetical_4gridh.cdt";
		...
		for (arg = 0; arg < astring.length; arg++) {
			...
			if (astring[arg].equals("-r")) {
				...
				if (astring.length > arg + 1) {
					System.out.println("In the next if...");
					System.out.println(arg + " " + astring[1]);
					sFilePath = astring[arg + 1];
					arg++;
				}
			}
		...
		if (sFilePath != null) {
			...
			if (sFilePath.startsWith("http://")) {
				...
				System.out.println("The fileset is: " + fileSet);
				System.out.println("Fileset information: " + file.getName() + " ||||| " + file.getParent()+File.separator);
			}
			...
			*/
			try {
				...
				e.printStackTrace();
				System.out.println("Load Error. " + fileSet.toString());
			}
		} else {
	...
	public static void main(String astring[]) {
		if(tvFrame != null){
			...
			statView  = new LinkedViewApp();
			statView.dealWithRegistration();
			// setup toplevel
			statView.magicStartup(astring);
		//	statView.windows.add(tvFrame);
		//	static LinkedViewFrame tvFrame = null; 
		// 	static LinkedViewApp statView = null;
		}	
		File file = new File(astring[1]);
		FileSet fileSet = new FileSet(file.getName(), file.getParent()+File.separator);
		
		try{

			statView.tvFrame.loadFileSet(fileSet);
			fileSet = statView.tvFrame.fileMru.addUnique(fileSet);
			statView.tvFrame.fileMru.setLast(fileSet);
			statView.tvFrame.fileMru.notifyObservers();
			statView.tvFrame.setLoaded(true);
	
		}catch(Exception e){
			System.out.println("Error opening file: " + fileSet);
		}
		try{
